...
echo "             Start     End       Size"
arm-none-eabi-nm -S -s $1.elf -t d| awk 'BEGIN {FS = " "; bootstart=134217728;vecstart=0;resetstart =0;resetend=etextstart=0;etextend=0;edata=0;} {if ($3 == "g_pfnVectors"){vecstart=$1};if($3 == "_etext"){etextend=$1};if ($3 == "__reset_data_start"){resetstart=$1};if ($3 == "__reset_data_end"){resetend=$1};if ($3 == "__etext_start"){etextstart=$1};} END{printf("Bootloader %8x %8x %6x (%dKb)\n", bootstart, vecstart-1, vecstart-bootstart, (vecstart-bootstart)/1024);printf("Vectors    %8x %8x %6x (%dKb)\n", vecstart, etextstart-1, etextstart - vecstart, (etextstart - vecstart)/1024);printf("Reset data %8x %8x %6x (%dKb)\n", resetstart, resetend-1, resetend-resetstart, (resetend-resetstart)/1024);printf("Text       %8x %8x %6x (%dKb)\n", etextstart, etextend, etextend-etextstart, (etextend-etextstart)/1024);}'
