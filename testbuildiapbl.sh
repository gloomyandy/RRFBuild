...
if [ -d boards/${DIR}/${BOARD} ]; then
  ...
  make -j8 PROJECT=IAP CORE=${CORE} MCU=${MCU} CONFIG=IAP_BOOT_LOADER USE_SD=${USE_SD} USE_CAN=${USE_CAN} SDTYPE=${SDTYPE} LED_PIN=${LED} CAN_READ=${CANREAD} CAN_WRITE=${CANWRITE} CAN_ADDRESS=${CAN_ADDRESS} BOARD_NAME=${BOARD} VARIANT=${VARIANT} OUTPUT_NAME=${OUTNAME} all
else
